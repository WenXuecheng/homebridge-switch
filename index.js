...
function switch_on_raspberry(switch_name, switch_option, log) {
    return new Promise(resolve => {
        ...
        http.get('http://localhost:6526/homebridge/switch/' + switch_name + '/' + switch_option + "/", (res) => {
...
class AccessoryPluginSwitch {
    ...
    async getOnHandler() {
        ...
        let s = await switch_on_raspberry(this.config.name, 'get_status', this.log);
        s = s.status_switch;
        const value = s;
    ...
    async setOnHandler(value) {
        ...
        if (value)
            ...
            op = 'close';
        let re = await switch_on_raspberry(this.config.name, op, this.log);
        this.log.info(re);
    }
}
